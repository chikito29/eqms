            ...
            <div class="x-content" style="min-height: 90vh;">
                ...
                <div class="x-content-title" style="margin-bottom:60px;">
                    ...
                    <div class="pull-right">
                        ...
                        <button class="btn btn-default btn-condensed @if(App\EqmsUser::isAdmin(request('user.id')) || App\EqmsUser::isMainDocumentController(request('user.id')) || App\HelperClasses\User::isSuperAdmin(request('user.id'))) @else hide @endif" onclick="location.href = '{{ route('documents.edit', $document->id) }}';"><i class="glyphicon glyphicon-edit"></i></button>
                        <button class="btn btn-default btn-condensed @if(App\EqmsUser::isAdmin(request('user.id')) || App\HelperClasses\User::isSuperAdmin(request('user.id'))) @else hide @endif" onclick="delete_document({{ $document->id }})"><i class="glyphicon glyphicon-trash"></i></button>
